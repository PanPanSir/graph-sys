...
export class PortService {
  ...
  async modify(req: UpdatePortDto) {
    ...
    const nodeViewType = vsNode.viewType;
    ...
    if (vsPortType === VsPortTypeEnum.OUTPUT_PORT) {
      if (propScript !== undefined && propScript !== '') {
        ...
        if (nodeTaskType === VsNodeTaskTypeEnum.COMPOSITE_NORMAL
          ...
          if (path !== undefined && path !== '') {
            // 检查context path是否重复
            const otherProjects = await this.prismaService.t_vs_project.findMany({
              where: {
                contextPath: path,
                id: {
                  not: projectId
            ...
            if (count > 0) {
              const names = otherProjects.map(project => project.name).join(',');
              throw new BadRequestException(
            ...
            console.error(`path prop can NOT be empty, portId = ${portId}, nodeId = ${vsNode.id}, taskType = ${vsNode.taskType}`);
            throw new BadRequestException(
      ...
      console.error(`unsupported port type, portId = ${portId}, Type = ${vsPortType}`);
      throw new BadRequestException(`不支持的端口类型,端口ID=${portId},端口类型=${vsPortType}`);
    }
   ...
   */
   generatePortScriptWhenRouteTaskType(
      ...
      // 例如，如果sourceType是REQ_HEADER，key是"Content-Type"，那么生成的leftValueExp将是： "this.getRequestHeader().get(\"Content-Type\")"
      const sourceData = VsPortRouteMetaSourceTypeExpression[sourceType];
      const leftValueExp = `${sourceData}.get("${key}")`;
      ...
      if (dataType === VsPortRouteMetaDataTypeEnum.INTEGER) {
        ...
        console.error(`unsupported dataType, portId = ${portId}, opType = ${dataType}`);
        throw new Error(
